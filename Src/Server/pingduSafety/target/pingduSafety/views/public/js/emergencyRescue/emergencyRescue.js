...
function searchRescue(page) {
	...
	$.ajax({
		...
		type: "GET",
		data: { "searchType":type,"keyword": $("#keyword").val(), "page": page },
		dataType: "JSON",
		success: function (result) {
			console.log(result);
			//console.log(page+"hahahahah");
...
//修改应急救援库信息                                                                                         JSON格式不正确？？？
function modifyRescue(rescueCode) {
	...
	var a = new sslAlert({
		...
		"callback": function () {
			...
			$.ajax({
				...
				dataType: 'JSON',//应该是Json格式
				success: function (result) {
					console.log(result);
					if (result == "SUCCESS") {
						//console.log("哈哈哈哈哈哈哈111");
						var b = new sslAlert({
							"titleName": "修改成功！",
							"cancleBtn": "取消",
							"okBtn": "确认"
	...
	var a = new sslAlert({
		...
		"callback": function () {
			...
			$.ajax({
				...
				type: "POST",
				data: {"emeRescue":emeRescue,"page":1,"lastpage":1},//？？？？,emeRescue错误
				dataType: 'JSON',
				success: function (result) {
...
$('.save-btn').on('click',function(){
	...
	window.parent.showDialog('保存',function(){
